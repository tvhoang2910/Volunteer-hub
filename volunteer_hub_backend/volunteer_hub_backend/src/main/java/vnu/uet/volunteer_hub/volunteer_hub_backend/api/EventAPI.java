...
public class EventAPI {
    ...
    public ResponseEntity<?> updateEvent(
        ...
        try {
            ...
            EventResponseDTO response = eventService.updateEvent(eventId, request, updaterId);
        ...
        try {
            ...
            List<EventSimpleDTO> events = eventsList.stream()
                    .map(e -> new EventSimpleDTO(
                            ...
                            e.getLocation(),
                            e.getDescription(),
                            e.getAdminApprovalStatus().toString(),
                            e.getStartTime(),
                            e.getEndTime(),
                            e.getThumbnailUrl()))
    ...
    /**
     * Get events created by the current manager.
    ...
    public ResponseEntity<?> getMyEvents() {
        try {
            // Get userId from SecurityContext (JWT filter sets UUID as principal)
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            UUID userId = (UUID) auth.getPrincipal();

            System.out.println("=== getMyEvents called ===");
            System.out.println("User ID: " + userId);

            List<Event> eventsList = eventService.getEventsByManager(userId);
            System.out.println("Found " + eventsList.size() + " events");

            List<EventSimpleDTO> events = eventsList.stream()
                    .map(e -> {
                        System.out.println("Event: " + e.getTitle() + ", Status: " + e.getAdminApprovalStatus());
                        return new EventSimpleDTO(
                                e.getId().toString(),
                                e.getTitle(),
                                e.getLocation(),
                                e.getDescription(),
            ...
            ResponseDTO<List<EventSimpleDTO>> responseDTO = ResponseDTO.<List<EventSimpleDTO>>builder()
                    .message("My events retrieved successfully")
                    .data(events)
                    .build();

            System.out.println("Response DTO: " + responseDTO);
            System.out.println("Response data size: " + events.size());
    ...
    public ResponseEntity<?> deleteEvent(@PathVariable UUID eventId) {
        try {
            ...
            // Check if event exists and user is the creator (unless admin)
            Event event = eventService.getEventEntityById(eventId);
            boolean isAdmin = auth.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));

            if (!isAdmin && !event.getCreatedBy().getId().equals(userId)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(ResponseDTO.builder()
