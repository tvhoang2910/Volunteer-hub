...
import CreatePostForm from "./createPostBar";
      ...
      <div className="flex flex-col rounded-lg bg-white p-3 px-4 shadow dark:bg-neutral-800">
        <div className="mb-2 flex items-center space-x-2 border-b pb-3 dark:border-neutral-700">
          <div className="h-10 w-10">
            <img src={userAvatar} className="h-full w-full rounded-full object-cover" alt="avatar" />
          </div>
          <button
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-3xl p-4">
            {/* You can pass user data to CreatePostForm via props if you update that component */}
            <CreatePostForm />

            {/* Close button (in case CreatePostForm doesn't include its own close) */}
            <button
              onClick={closePostBar}
              className="absolute top-3 right-3 rounded-full bg-white/90 p-1 shadow hover:bg-white"
              aria-label="Đóng"
            >
              ✕
            </button>
